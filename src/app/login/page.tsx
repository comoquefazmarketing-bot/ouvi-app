...
// --- MOTOR DE PARTÍCULAS (CORRIGIDO PARA THREE.JS NATIVO) ---
function ParticlesLogo() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      const i3 = i * 3;
      posArr[i3 + 1] += Math.sin(t + posArr[i3]) * 0.002;
    }
  ...
  return (
    <points ref={pointsRef}>
      ...
      {/* CORREÇÃO AQUI: pointsMaterial minúsculo é nativo do R3F/Three */}
      <pointsMaterial 
        size={0.015} 
        vertexColors 
        transparent 
    ...
    <div style={styles.container} onClick={playSensorialSound}>
      ...
      <div style={styles.content}>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2 }}>
          <img src="/logo-ouvi.svg" alt="OUVI" style={{ width: "200px", height: "auto" }} />
        ...
        <div style={styles.buttonGroup}>
          <motion.button whileHover={{ scale: 1.02 }} onClick={() => handleLogin('google')} style={styles.premiumBtn}>
            <span style={styles.btnText}>ENTRAR COM GOOGLE</span>
          </motion.button>
        </div>
...
const styles = {
  ...
  tagline: { color: "#fff", fontSize: "9px", letterSpacing: "8px", opacity: 0.4, marginTop: "20px", fontWeight: "900" },
  buttonGroup: { display: "flex", flexDirection: "column" as const, gap: "15px", marginTop: "80px" },
  premiumBtn: { background: "rgba(255, 255, 255, 0.05)", border: "1px solid rgba(255, 255, 255, 0.1)", padding: "20px", borderRadius: "2px", cursor: "pointer", color: "#fff", backdropFilter: "blur(10px)" },
